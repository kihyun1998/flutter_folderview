...
- **note**: when a tree is wide enough to scroll horizontally, a long label's rect extends past the `FolderView`, so with the default `TooltipAnchor.child` its tooltip may be painted outside the view. Pre-existing, not introduced here; the fix belongs in `just_tooltip`, which computes the target rect. Tracked in [#47](https://github.com/kihyun1998/flutter_folderview/issues/47). `TooltipAnchor.pointer` avoids it — the cursor is inside the view by construction.
