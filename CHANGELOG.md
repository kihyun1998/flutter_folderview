...
- **feat**: `NodeTooltipTheme.anchor` ([#42](https://github.com/kihyun1998/flutter_folderview/issues/42)). A node's tooltip attaches to its icon-and-label content. A label long enough to ellipsize has consumed all the width available to it, so its rect spans the row — and a tooltip anchored to that rect appears at its centre, which can be far from where the user is actually pointing. `anchor: TooltipAnchor.pointer` keeps the same hover region but places the tooltip at the cursor. `TooltipAnchor` is now re-exported. Defaults to `TooltipAnchor.child`, the behaviour shipped in `0.10.2`, so existing tooltips are unchanged.
  ...
  - `anchor` does not widen the hover region — a short label leaves the space to its right raising no tooltip under either anchor. Row-wide tooltips are tracked in [#44](https://github.com/kihyun1998/flutter_folderview/issues/44).
- **note**: `TooltipRegistry` (added in `just_tooltip 0.3.0`) is still not re-exported. Since `just_tooltip 0.4.0` made a nested tooltip suppress its ancestors regardless of registry, the only remaining use for a separate registry is isolating a `FolderView`'s one-tooltip-at-a-time group from the rest of the host app. Tracked in [#42](https://github.com/kihyun1998/flutter_folderview/issues/42).
