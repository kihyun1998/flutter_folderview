...
- **note**: `just_tooltip 0.4.0` adds `TooltipAnchor`, which anchors a tooltip at the pointer instead of the child's rect. It is deliberately **not** re-exported yet: `NodeTooltipTheme` has no `anchor` field, so the enum would be visible but unusable. Exposing it is a `flutter_folderview` feature decision — see the follow-up issue.
