...
- **M4, stale forward prose.** [ADR-0003](../adr/0003-selection-is-tier-bound-to-child.md)'s implementation note predicts a removal that has already happened (`bfb91db`), and it recommends `onSecondaryNodeTap` for containers, which the code does not deliver ([tap-and-selection](territory/tap-and-selection.md), probed). ADRs are not edited from the map.
- **M5, backlog.** One open issue, #16. Its body is partly stale ([theme-composition](territory/theme-composition.md)).
- **Probed at build:** 3 holes, each against a control. Two violate [measured-equals-rendered](invariant/measured-equals-rendered.md), and one is the ADR-0003 gap above. None filed.

## Coverage and what an absent note means
Covered: all of `lib/`, CI, the dependency floor and release. **Not covered: `example/`.** It is being replaced, and a note written now would describe code about to be deleted. **A territory for the example is owed when the new example lands**, and not before. Until then, the absence of an example note is correct, not a backlog item.
